...
class SOVTrack(nn.Module):
    ...
    def forward(self,
        ...
        for i in range(len(template_image)):
            img = template_image[i]
            evt = template_event[i]
            template.append(self.fusion(img, evt))

        for i in range(len(search_image)):
            img = search_image[i]
            evt = search_event[i]
            search.append(self.fusion(img, evt))
...
def build_model(cfg, training=True):
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit_base_patch16_224':
        ...
        backbone = vit_base_patch16_224_ce(pretrained, drop_path_rate=cfg.TRAIN.DROP_PATH_RATE,
                                           ...
                                           add_cls_token=cfg.MODEL.BACKBONE.ADD_CLS_TOKEN,
                                           in_chans = cfg.MODEL.BACKBONE.DIM * 2,
                                           # in_chans = 3
                                           )
