...
from .CFA import CrossFusionAttention
_logger = logging.getLogger(__name__)


class VisionTransformerCE(VisionTransformer):
    def __init__(self, img_size=224, patch_size=16, in_chans=64, num_classes=1000, embed_dim=768, depth=12,
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        lens_z = z.shape[1]  # HW  1, 432, 768
        lens_x = x.shape[1]  # HW  1, 576, 768
        ...
        for i, blk in enumerate(self.blocks):
            if self.add_cls_token:
                x, z, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, z, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate, add_cls_token=self.add_cls_token, query_len=query_len)
            else:
                x, z, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, z, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate, add_cls_token=self.add_cls_token)
